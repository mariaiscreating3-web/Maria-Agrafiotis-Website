    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="mb-8 flex items-end justify-between">
        <div>
          ...
          <h1 style={{ fontFamily: "var(--font-cormorant)", fontSize: "2.8rem", fontWeight: 300, lineHeight: 1 }}>
            All Posts
          </h1>
        </div>
        <Link
      ...
      <div className="flex gap-0 border-b border-border mb-6">
        {(["all", "published", "draft"] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className="px-5 py-2.5 bg-transparent border-0 cursor-pointer transition-all"
            style={{
              ...labelStyle,
              borderBottom: filter === f ? "2px solid var(--burgundy)" : "2px solid transparent",
              color: filter === f ? "var(--burgundy)" : "inherit",
      ...
      {filtered.length === 0 ? (
        <div className="border border-dashed border-border p-12 text-center">
          <p style={{ fontFamily: "var(--font-cormorant)", fontSize: "1.3rem", opacity: 0.5 }}>
            No posts here yet
          </p>
        </div>
      ) : (
        <div className="border-t border-border">
          {filtered.map((post) => {
            ...
            return (
              <div key={post.id} className="group flex items-center gap-4 py-4 border-b border-border hover:bg-card/50 px-1 transition-colors -mx-1">
                {/* Status dot */}
                <div
                  className="w-1.5 h-1.5 rounded-full shrink-0"
                  style={{ background: post.published ? "var(--burgundy)" : "var(--border)" }}
                />
                ...
                <div className="flex-1 min-w-0">
                  ...
                  >
                    ...
                    {post.featured && (
                      <span
                        className="ml-2"
                        style={{ fontFamily: "var(--font-raleway)", fontSize: "0.52rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--burgundy)", verticalAlign: "middle" }}
                      >
                        ★ Featured
                      </span>
                    )}
                  ...
                  <p style={{ ...labelStyle, opacity: 0.45, fontSize: "0.57rem" }}>
                    {post.date} · {cat?.label ?? post.category}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <button
                    ...
                    className="px-2.5 py-1 border bg-transparent cursor-pointer transition-all"
                    style={{
                      ...labelStyle,
                      fontSize: "0.55rem",
                      borderColor: post.published ? "var(--burgundy)" : "var(--border)",
                      color: post.published ? "var(--burgundy)" : "inherit",
                  ...
                  <button
                    ...
                    className="bg-transparent border-0 cursor-pointer p-1 opacity-30 hover:opacity-70 transition-opacity"
                  >
